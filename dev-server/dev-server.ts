...
const usersGetById = require("../lambdas/users/get");
const usersCreateUpdate = require("../lambdas/users/create-update");

app.get(
  "/users",
  async (request: express.Request, response: express.Response) => {
    const data = await processRequest(usersGetById, request);
    response.status(data.statusCode).send(JSON.parse(data.body));
  }
);
...
app.post(
  "/users",
  async (request: express.Request, response: express.Response) => {
    const data = await processRequest(usersCreateUpdate, request);
    response.status(data.statusCode).send(JSON.parse(data.body));
  }
);
